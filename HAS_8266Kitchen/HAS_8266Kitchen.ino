...
void MQTTcallback(char* topic, byte* payload, unsigned int length)
{
  ...
  for (i=0 ; i<sizeof(topics_sub)/sizeof(topics_sub[0]) ; i++) 
  {
    if( !strcmp(topic, topics_sub[i].c_str() ) )
    {
      ...
      {
         digitalWrite(topics_pins[i], true); // relay is closed on 0, open on 1 (ie please invert message)
        Serial.print("(OFF)");
      }
      else
      {
         digitalWrite(topics_pins[i], false); // relay is closed on 0, open on 1 (ie please invert message)        
        Serial.print("(ON)");
      }
    }
  }
